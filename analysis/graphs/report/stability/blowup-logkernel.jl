...
ax1 = Axis(
    ...
    title="LogKernel stability of forward-inplace, n = 10",
);

b = lines!(ax1, real(x), baseline_log, linewidth=2);
i = lines!(ax1, real(x), logtransform(10), linewidth=2);
...
i = lines!(ax2, real(x), logtransform(100), linewidth=2);

ax3 = Axis(
    fig[3, 1],
    xlabel=L"z",
    ylabel=L"\mathcal{L}[exp](z)",
    title="LogKernel stability of forward-inplace, n = 1000",
);

b = lines!(ax3, real(x), baseline_log, linewidth=2);
i = lines!(ax3, real(x), logtransform(1000), linewidth=2);
