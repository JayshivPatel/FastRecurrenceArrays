...
baseline = collect([Float32.(log.(abs.(x₀ .- axes(P, 1)')) * f_N) for x₀ in x]);

ff = Float32.(collect(f_N.args[2][1:T])); fixed .= FixedLogKernel(T, x, ff);
f_g(x, z) = log(z-x)*exp(x); x_g, w_g = gausslegendre(T); gauss .= [real(dot(w_g, f_g.(x_g, x₀))) for x₀ in x];
