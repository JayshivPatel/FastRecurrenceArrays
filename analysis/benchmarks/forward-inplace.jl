...
println(median(@benchmark ForwardInplace($params...) samples = 100 seconds = 100));

println("ThreadedInplace - " * string(N) * " recurrences on " * string(M) * " points.");
println("Threads - " * string(Threads.nthreads()));
println(median(@benchmark ThreadedInplace($params...) samples = 100 seconds = 100));

println("GPUInplace - " * string(N) * " recurrences on " * string(M) * " points.");
println(median(@benchmark GPUInplace($params...) samples = 100 seconds = 100));