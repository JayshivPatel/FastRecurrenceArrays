...
println(median(@benchmark FixedRecurrenceArray($params...) samples = 100));

println("ThreadedRecurrenceArray (row-wise) - " * string(N) * " recurrences on " * string(M) * " points.");
println("Threads - " * string(Threads.nthreads()));
println(median(@benchmark ThreadedRecurrenceArray($params..., Val(1)) samples = 100));

println("ThreadedRecurrenceArray (column-wise) - " * string(N) * " recurrences on " * string(M) * " points.");
println("Threads - " * string(Threads.nthreads()));
println(median(@benchmark ThreadedRecurrenceArray($params..., Val(2)) samples = 100));

println("GPURecurrenceArray - " * string(N) * " recurrences on " * string(M) * " points.");
println(median(@benchmark GPURecurrenceArray($params...) samples = 100));