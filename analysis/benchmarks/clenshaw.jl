...
println(median(@benchmark FixedClenshaw($params[1], $params[2], $(collect(params[3]))) samples = 100 seconds = 500));

println("ThreadedClenshaw - " * string(N) * " recurrences on " * string(M) * " points.");
println("Threads - " * string(Threads.nthreads()));
println(median(@benchmark ThreadedClenshaw($params...) samples = 100 seconds = 500));

println("GPUClenshaw - " * string(N) * " recurrences on " * string(M) * " points.");
println(median(@benchmark GPUClenshaw($params...) samples = 100 seconds = 500));