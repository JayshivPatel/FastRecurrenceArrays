...
export FixedStieltjes, GPUStieltjes, InplaceLogKernel, GPULogKernel
...
function FixedStieltjes(n::Integer, x::AbstractVector, f::AbstractVector, P::OrthogonalPolynomial=Legendre())
    ...
    p₀ = real(stieltjes(w, x) .* _p0(P))
    p₁ = real((A[1] .* x .+ B[1]) .* p₀ .- (A[1]sum(w) * _p0(P)))

    return FixedClenshaw(f, A, B, C, real(x), p₀, p₁)
end

function GPUStieltjes(n::Integer, x::AbstractVector, f::AbstractVector, P::OrthogonalPolynomial=Legendre())
    w = orthogonalityweight(P)
    A, B, C = recurrencecoefficients(P)
    A, B, C = A[1:n], B[1:n], C[1:n]
    ...
    return GPUClenshaw(f, A, B, C, x, p₀, p₁)
end
...
function GPULogKernel(n::Integer, x::AbstractVector, f::AbstractVector)
    A, B, C = recurrencecoefficients(Ultraspherical(-1/2))
    A, B, C = A[2:n], B[2:n], C[2:n]
    ...
    return GPUInplace(f, (A, B, C), real(x), [p₀'; p₁'; p₂'])
end